...
<html>
    <head>
        ...
        <%
            ...
            try {
                statement.execute();
                session.setAttribute("message", "Speaker successfully assigned to Session!");
            }
            catch (Exception e) {
                //If the insert fails, it's a primary key violation, so display that message gracefully
                session.setAttribute("message", "Speaker has already been assigned to that Session");
            }
            finally {
                statement.close();
                connection.close();
            }
            response.sendRedirect("../admin/assignspeaker.jsp");
        %>
</html>